...
uniform sampler2D hx_AlbedoMap;
uniform sampler2D hx_RoughnessMap;
uniform sampler2D hx_MetallicMap;
uniform sampler2D hx_NormalMap;
uniform vec3 eyePos;
uniform samplerCube hx_IrradianceMap;
uniform samplerCube hx_PerfilterEnvMap;
uniform sampler2D hx_IntegrateBRDFMap;
...
vec3 calc_ibl(vec3 n, vec3 v, vec3 albedo, float roughness, float metalic) {
    ...
    vec3 ld = filtering_cube_map_lod(hx_PerfilterEnvMap, r, roughness);
    vec2 dfg = textureLod(hx_IntegrateBRDFMap, vec2(ndotv, roughness), 0.0).xy;
...
vec3 calc_normal() {
	...
	normal = texture2D(hx_NormalMap, vs_Texcoord).xyz;
...
void main() {    
    ...
    vec3 albedo = texture(hx_AlbedoMap, vs_Texcoord).xyz;
    float roughness = texture(hx_RoughnessMap, vs_Texcoord).x;
    float metallic = texture(hx_MetallicMap, vs_Texcoord).x;

    vec3 normal = calc_normal();
    vec3 colorDirect = calc_lighting_direct(normalize(normal), view, light, half1, albedo, roughness, metallic, vec3(2.5, 2.5, 2.5));
