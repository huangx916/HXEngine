...
namespace HX3D
{
	HXGLMaterial::HXGLMaterial(HXMaterialInfo* pMatInfo)
	{
		...
		if (pMatInfo->strShaderFile == "builtin/IBL")
		{
			isIBL = true;
			m_CubeMapRT = new HXGLRenderTarget(1024, 1024);
			m_CubeMap = new HXGLTexture();
			m_CubeMap->CreateFloat16CubeTexture(1024, 1024);
			DrawColorBuffer drawBuffer[6] = {
			...
			std::string strVertShaderFile1 = "builtin/filtering_ermap.vert";
			std::string strFragShaderFile1 = "builtin/filtering_ermap.frag";

			ShaderInfo shaders1[] = {
				{ GL_VERTEX_SHADER, strVertShaderFile1.c_str() },
				{ GL_FRAGMENT_SHADER, strFragShaderFile1.c_str() },
				{ GL_NONE, NULL }
			};

			render_target_prog = LoadShaders(shaders1);
			glUseProgram(render_target_prog);

			m_WVPLoc = glGetUniformLocation(render_target_prog, "glb_WVP");
			m_ERMapLoc = glGetUniformLocation(render_target_prog, "glb_EquirectangularMap");
	...
	void HXGLMaterial::SetMaterialRenderStateAllRenderable(HXICamera* curCamera)
	{
		...
		if (isIBL)
		{
			...
			glBindFramebuffer(GL_FRAMEBUFFER, reinterpret_cast<int32_t>(m_CubeMapRT->GetNativeRenderTarget()));
			...
			for (int32_t i = 0; i < 6; i++)
			{
				...
				glUniform1i(m_ERMapLoc, 0);
				glActiveTexture(GL_TEXTURE0);
				glBindTexture(GL_TEXTURE_2D, tex->texObj);

				// temporary
				glUniformMatrix4fv(m_WVPLoc, 1, GL_FALSE, vmath::mat4::identity());
