...
// Add by huangxin  N为矩阵列  M为矩阵行   K为向量行   ===>  N == K
template <typename T, const int N, const int M, const int K>
static inline vecN<T, K> operator*(const matNM<T, N, M>& mat, const vecN<T, K>& vec)
{
	int n, m;
	vecN<T, N> result(T(0));
